...
models_output="$(HOME="$temp_home" PI_CODING_AGENT_DIR="$agent_dir" pi --list-models cursorapi 2>&1)"
printf '%s\n' "$models_output"
grep -F "cursorapi  composer-2.5" <<<"$models_output" >/dev/null || fail "pi did not list composer-2.5"
